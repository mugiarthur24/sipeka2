...
<div class="mt-4">
  <div class="card">
    ...
    <div class="card-body">
    	...
    	<?php endif ?>
      <form action="<?php echo base_url('index.php/admin/cspensiun/pensiunjandaduda_action/'.$pegawai->id_pegawai) ?>" method="post" enctype='multipart/form-data'>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><input type="file" name="upload_24"></td>
                 <td>
                  <?php if (@$formupload->upload_24 == TRUE): ?>
                    <a href="<?php echo base_url('asset/dokumen/'.$formupload->upload_24) ?>" target="_blank" class="btn btn-success btn-sm w-100">View</a>
                    <?php else: ?>
                      <span class="btn btn-secondary btn-sm w-100">View</span>
                  <?php endif ?>
              </td>
                <td>
                <?php if (@$formupload->verifikasi_24 == TRUE && @$formupload->verifikasi_24 !==0): ?>
                  <?php $veri24 = $this->Admin_m->detail_data_order('status','id_status',$formupload->verifikasi_24); ?>
                <span class="<?php echo $veri24->kode_status; ?>"><?php echo $veri24->nm_status; ?></span>
                <?php else: ?>
                  <span class="danger">tidak ada file</span>
                <?php endif ?>
              </td>
               <td>
                <select name="action24">
                  <option value="<?php $formupload->verifikasi_24 ?>">-- <?php echo @$this->Admin_m->detail_data_order('status','id_status',$formupload->verifikasi_24)->nm_status; ?> --</option>
                  <?php foreach ($dtsts as $key): ?>
                    <option value="<?php echo $key->id_status ?>"><?php echo $key->nm_status ?></option>
                  <?php endforeach ?>
                </select>
                <textarea name="ket24" placeholder="keterangan"><?php echo $formupload->ket_24; ?></textarea>
              </td>
              </tr>
              <tr>
                <th scope="row">25</th>
                <td>SURAT KETERANGAN JANDA/DUDA DARI KELUARAHAN/DESA</td>
                <td><input type="file" name="upload_25"></td>
                 <td>
                  <?php if (@$formupload->upload_25 == TRUE): ?>
                    <a href="<?php echo base_url('asset/dokumen/'.$formupload->upload_25) ?>" target="_blank" class="btn btn-success btn-sm w-100">View</a>
                    <?php else: ?>
                      <span class="btn btn-secondary btn-sm w-100">View</span>
                  <?php endif ?>
              </td>
                <td>
                <?php if (@$formupload->verifikasi_25 == TRUE && @$formupload->verifikasi_25 !==0): ?>
                  <?php $veri25 = $this->Admin_m->detail_data_order('status','id_status',$formupload->verifikasi_25); ?>
                <span class="<?php echo $veri25->kode_status; ?>"><?php echo $veri25->nm_status; ?></span>
                <?php else: ?>
                  <span class="danger">tidak ada file</span>
                <?php endif ?>
              </td>
               <td>
                <select name="action25">
                  <option value="<?php $formupload->verifikasi_25 ?>">-- <?php echo @$this->Admin_m->detail_data_order('status','id_status',$formupload->verifikasi_25)->nm_status; ?> --</option>
                  <?php foreach ($dtsts as $key): ?>
                    <option value="<?php echo $key->id_status ?>"><?php echo $key->nm_status ?></option>
                  <?php endforeach ?>
                </select>
                <textarea name="ket25" placeholder="keterangan"><?php echo $formupload->ket_25; ?></textarea>
              </td>
              </tr>
            </tbody>
          </table>
