...
class Cspensiun extends CI_Controller {
    ...
    public function pensiunjandaduda_main(){
        if ($this->ion_auth->logged_in()) {
            $level = array('admin','members');
            if (!$this->ion_auth->in_group($level)) {
                $pesan = 'Anda tidak memiliki Hak untuk Mengakses halaman ini';
                ...
                $datauser = $this->ion_auth->user()->row();
                if ($this->ion_auth->in_group('members')) {
                    redirect( base_url('index.php/admin/cspensiun/pensiunjandaduda_tambah/'.$datauser->id_pegawai));
                }
                    // echo "<pre>";print_r($datauser->cs_mutasi);echo "<pre/>";exit();
                $post = $this->input->get();
                $data['title'] = 'Layanan Pensiun Janda/Duda '.$this->Admin_m->info_pt(1)->nama_info_pt;
                $data['infopt'] = $this->Admin_m->info_pt(1);
                $data['brand'] = 'asset/img/lembaga/'.$this->Admin_m->info_pt(1)->logo_pt;
                $data['users'] = $datauser;
                $data['pangkat'] = $this->Admin_m->select_data('master_pangkat');
                ...
                $data['jabatan'] = $this->Admin_m->select_data('master_jabatan');
                $data['skpd'] = $this->Admin_m->select_data('master_lokasi_kerja');
                $data['hasil'] = $this->Admin_m->select_data('form_pensiunjandaduda');
                $data['aside'] = 'nav/nav';
                $data['page'] = 'admin/cs/pensiun-jandaduda-main-v';
                                    // pagging setting
                $this->load->view('admin/dashboard-v',$data);
            }
        }else{
    ...
    public function pensiunjandaduda_tambah($idpegawai){
        if ($this->ion_auth->logged_in()) {
            $level = array('admin','members');
            if (!$this->ion_auth->in_group($level)) {
                $pesan = 'Anda tidak memiliki Hak untuk Mengakses halaman ini';
                ...
                $datauser = $this->ion_auth->user()->row();
                $pegawai = $this->Admin_m->detail_data_order('data_pegawai','id_pegawai',$idpegawai);
                $cekfomupload = $this->Admin_m->detail_data_order('form_pensiunjandaduda','id_pegawai',$pegawai->id_pegawai);
                    $post = $this->input->get();
                    $data['title'] = 'Pensiun Janda/Duda'.$this->Admin_m->info_pt(1)->nama_info_pt;
                    $data['infopt'] = $this->Admin_m->info_pt(1);
                    $data['brand'] = 'asset/img/lembaga/'.$this->Admin_m->info_pt(1)->logo_pt;
                    $data['users'] = $datauser;
                    $data['pangkat'] = $this->Pensiun_m->last_pangkat($idpegawai);
    ...
    public function caripegawaipensiunjandaduda(){
        $post = $this->input->post();
        $cari = $post["string"];
        // echo $post['string'];
        $hasil = $this->Pensiun_m->cari_pegawaipensiunjandaduda($cari);
        if ($hasil == TRUE) {
            foreach ($hasil as $data) {
    ...
    public function pensiunpidana2($nip){
        ...
        if ($this->ion_auth->logged_in()) {
            ...
            if (!$this->ion_auth->in_group($level)) {
                ...
                $getpegawai = $this->Admin_m->detail_data_order('data_pegawai','id_pegawai',$nip);
                $cekfomupload = $this->Admin_m->detail_data_order('form_pensiunpidana','id_pegawai',$getpegawai->id_pegawai);
                if ($cekfomupload == FALSE) {
                    $dps = array(
                        'id_pegawai'=>$getpegawai->id_pegawai,
                        'tgl_create'=>date('Y-m-d')
                    );
                    $this->Admin_m->insert_data('form_pensiunpidana',$dps);
                }
                $cekfomupload = $this->Admin_m->detail_data_order('form_pensiunpidana','id_pegawai',$getpegawai->id_pegawai);
                // echo "<pre>";print_r($cekfomupload);echo "<pre>";exit();
                $pesaneror = array();
                ...
                // upload 2
                if (!empty($_FILES["upload_2"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-2'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 3
                if (!empty($_FILES["upload_3"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-3'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 4
                if (!empty($_FILES["upload_4"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-4'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 5
                if (!empty($_FILES["upload_5"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-5'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 6
                if (!empty($_FILES["upload_6"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-6'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 7
                if (!empty($_FILES["upload_7"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-7'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 8
                if (!empty($_FILES["upload_8"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-8'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 9
                if (!empty($_FILES["upload_9"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-9'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 10
                if (!empty($_FILES["upload_10"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-10'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 11
                if (!empty($_FILES["upload_11"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-11'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 12
                if (!empty($_FILES["upload_12"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-12'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 13
                if (!empty($_FILES["upload_13"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-13'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 14
                if (!empty($_FILES["upload_14"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-14'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 15
                if (!empty($_FILES["upload_15"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-15'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 16
                if (!empty($_FILES["upload_16"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-16'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 17
                if (!empty($_FILES["upload_17"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-17'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 18
                if (!empty($_FILES["upload_18"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-18'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 19
                if (!empty($_FILES["upload_19"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-19'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 20
                if (!empty($_FILES["upload_20"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-20'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 21
                if (!empty($_FILES["upload_21"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunpidana'.'-'.$getpegawai->nip.'-21'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
    ...
    public function pensiunjandaduda2($nip){
        $post = $this->input->post();
        if ($this->ion_auth->logged_in()) {
            $level = array('admin','members');
            if (!$this->ion_auth->in_group($level)) {
                ...
                $getpegawai = $this->Admin_m->detail_data_order('data_pegawai','id_pegawai',$nip);
                $cekfomupload = $this->Admin_m->detail_data_order('form_pensiunjandaduda','id_pegawai',$getpegawai->id_pegawai);
                if ($cekfomupload == FALSE) {
                    $dps = array(
                        'id_pegawai'=>$getpegawai->id_pegawai,
                        'tgl_create'=>date('Y-m-d')
                    );
                    $this->Admin_m->insert_data('form_pensiunjandaduda',$dps);
                }
                $cekfomupload = $this->Admin_m->detail_data_order('form_pensiunjandaduda','id_pegawai',$getpegawai->id_pegawai);
                // echo "<pre>";print_r($cekfomupload);echo "<pre>";exit();
                $pesaneror = array();
                // upload 1
                if (!empty($_FILES["upload_1"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-1'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                    ...
                    if (!$this->upload->do_upload('upload_1')){
                        ...
                        $data['upload_1'] = $img;
                        $data['verifikasi_1'] = '1';
                        $this->Admin_m->update('form_pensiunjandaduda','id_form_pensiunjandaduda',$cekfomupload->id_form_pensiunjandaduda,$data);
                    }
                }
                // upload 2
                if (!empty($_FILES["upload_2"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-2'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 3
                if (!empty($_FILES["upload_3"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-3'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 4
                if (!empty($_FILES["upload_4"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-4'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 5
                if (!empty($_FILES["upload_5"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-5'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 6
                if (!empty($_FILES["upload_6"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-6'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 7
                if (!empty($_FILES["upload_7"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-7'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 8
                if (!empty($_FILES["upload_8"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-8'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 9
                if (!empty($_FILES["upload_9"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-9'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 10
                if (!empty($_FILES["upload_10"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-10'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 11
                if (!empty($_FILES["upload_11"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-11'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 12
                if (!empty($_FILES["upload_12"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-12'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 13
                if (!empty($_FILES["upload_13"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-13'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 14
                if (!empty($_FILES["upload_14"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-14'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 15
                if (!empty($_FILES["upload_15"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-15'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 16
                if (!empty($_FILES["upload_16"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-16'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 17
                if (!empty($_FILES["upload_17"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-17'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 18
                if (!empty($_FILES["upload_18"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-18'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 19
                if (!empty($_FILES["upload_19"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-19'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 20
                if (!empty($_FILES["upload_20"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-20'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 21
                if (!empty($_FILES["upload_21"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-21'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 22
                if (!empty($_FILES["upload_22"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-22'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 23
                if (!empty($_FILES["upload_23"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-23'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 24
                if (!empty($_FILES["upload_24"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-24'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                ...
                // upload 25
                if (!empty($_FILES["upload_25"]["tmp_name"])) {
                    $config['file_name'] = strtolower(url_title('pensiunjandaduda'.'-'.$getpegawai->nip.'-25'.'-'.date('Ymd').'-'.time('Hms')));
                    $config['upload_path'] = './asset/dokumen/';
                    $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                    $config['max_size'] = 2048;
                    $config['max_width'] = '';
                    $config['max_height'] = '';
                    $this->load->library('upload', $config);
                    if (!$this->upload->do_upload('upload_25')){
                        $error = $this->upload->display_errors();
                        $pesaneror[]=$error;
                    }
                    else{
                        $img25 = $this->upload->data('file_name');
                        $data['upload_25'] = $img25;
                         $data['verifikasi_25'] = '1';
                        $this->Admin_m->update('form_pensiunjandaduda','id_form_pensiunjandaduda',$cekfomupload->id_form_pensiunjandaduda,$data);
                    }
                }
                $pesan = 'Form berhasil di upload';
                $this->session->set_flashdata('message', $pesan );
                redirect(base_url('index.php/admin/cspensiun/pensiunjandaduda_tambah/'.$getpegawai->id_pegawai));
            }
        }else{
            $pesan = 'Login terlebih dahulu';
            $this->session->set_flashdata('message', $pesan );
    ...
    public function detail_pensiunpidana($id){
        if ($this->ion_auth->logged_in()) {
            $level = array('admin','members');
            if (!$this->ion_auth->in_group($level)) {
                $pesan = 'Anda tidak memiliki Hak untuk Mengakses halaman ini';
                ...
                if ($this->ion_auth->in_group('members')) {
                    ...
                    // echo "<pre>";print_r($datauser->cs_mutasi);echo "<pre/>";exit();
                $post = $this->input->get();
                $datamutasi = $this->Admin_m->detail_data_order('form_pensiunpidana','id_form_pensiunpidana',$id);
                $datapegawai = $this->Admin_m->detail_data_order('data_pegawai','id_pegawai',$datamutasi->id_pegawai);
                $formupload = $this->Admin_m->detail_data_order('form_pensiunpidana','id_pegawai',$datapegawai->id_pegawai);
                $data['title'] = 'Detail Pensiun Tindak Pidana - '.$datapegawai->nip;
                $data['infopt'] = $this->Admin_m->info_pt(1);
                $data['brand'] = 'asset/img/lembaga/'.$this->Admin_m->info_pt(1)->logo_pt;
                $data['users'] = $datauser;
                $data['pegawai'] = $datapegawai;
                ...
                $data['dtsts'] = $this->Admin_m->select_data('status');
                $data['aside'] = 'nav/nav';
                $data['page'] = 'admin/cs/pensiun-pidana-detail-v';
                                    // pagging setting
                $this->load->view('admin/dashboard-v',$data);
            }
        }else{
    ...
    public function detail_pensiunjandaduda($id){
        if ($this->ion_auth->logged_in()) {
            $level = array('admin','members');
            if (!$this->ion_auth->in_group($level)) {
                $pesan = 'Anda tidak memiliki Hak untuk Mengakses halaman ini';
                ...
                if ($this->ion_auth->in_group('members')) {
                    ...
                    // echo "<pre>";print_r($datauser->cs_mutasi);echo "<pre/>";exit();
                $post = $this->input->get();
                $datamutasi = $this->Admin_m->detail_data_order('form_pensiunjandaduda','id_form_pensiunjandaduda',$id);
                $datapegawai = $this->Admin_m->detail_data_order('data_pegawai','id_pegawai',$datamutasi->id_pegawai);
                $formupload = $this->Admin_m->detail_data_order('form_pensiunjandaduda','id_pegawai',$datapegawai->id_pegawai);
                $data['title'] = 'Detail Pensiun Janda/Duda - '.$datapegawai->nip;
                $data['infopt'] = $this->Admin_m->info_pt(1);
                $data['brand'] = 'asset/img/lembaga/'.$this->Admin_m->info_pt(1)->logo_pt;
                $data['users'] = $datauser;
                $data['pegawai'] = $datapegawai;
                ...
                $data['dtsts'] = $this->Admin_m->select_data('status');
                $data['aside'] = 'nav/nav';
                $data['page'] = 'admin/cs/pensiun-jandaduda-detail-v';
                                    // pagging setting
                $this->load->view('admin/dashboard-v',$data);
            }
        }else{
    ...
    public function pensiunpidana_action($nip){
        if ($this->ion_auth->logged_in()) {
            $level = array('admin','members');
            if (!$this->ion_auth->in_group($level)) {
                $pesan = 'Anda tidak memiliki Hak untuk Mengakses halaman ini';
                ...
                if ($cekfomupload == TRUE) {
                    ...
                    // upload 10
                    if (!empty($_FILES["upload_10"]["tmp_name"])) {
                        $config['file_name'] = strtolower(url_title('pensiunpidanapensiunpidana'.'-'.$getpegawai->nip.'-10'.'-'.date('Ymd').'-'.time('Hms')));
                        $config['upload_path'] = './asset/dokumen/';
                        $config['allowed_types'] = 'pdf|jpg|png|jpeg';
                        $config['max_size'] = 2048;
                        $config['max_width'] = '';
                    ...
                    // echo "<pre>";print_r($datass);echo "<pre>";exit();
                    $this->Admin_m->update('form_pensiunpidana','id_form_pensiunpidana',$cekfomupload->id_form_pensiunpidana,$datass);
                }
                $pesan = 'Status dokumen berhasil diubah';
                $this->session->set_flashdata('message', $pesan );
                redirect(base_url('index.php/admin/cspensiun/detail_pensiunpidana/'.$cekfomupload->id_form_pensiunbup));
            }
        }else{
            $pesan = 'Login terlebih dahulu';
            $this->session->set_flashdata('message', $pesan );
    ...
    public function pensiunjandaduda_action($nip){
        if ($this->ion_auth->logged_in()) {
            $level = array('admin','members');
            if (!$this->ion_auth->in_group($level)) {
                $pesan = 'Anda tidak memiliki Hak untuk Mengakses halaman ini';
                ...
                if ($cekfomupload == TRUE) {
                    ...
                    if ($post['action25'] == TRUE) {
                         $datass['verifikasi_25'] = $post['action25'];
                    }
                    ...
                    $datass['ket_25'] = $post['ket25'];
                   
                    // echo "<pre>";print_r($datass);echo "<pre>";exit();
                    $this->Admin_m->update('form_pensiunjandaduda','id_form_pensiunjandaduda',$cekfomupload->id_form_pensiunjandaduda,$datass);
                }
                $pesan = 'Status dokumen berhasil diubah';
                $this->session->set_flashdata('message', $pesan );
                redirect(base_url('index.php/admin/cspensiun/detail_pensiunjandaduda/'.$cekfomupload->id_form_pensiunjandaduda));
            }
        }else{
            $pesan = 'Login terlebih dahulu';
            $this->session->set_flashdata('message', $pesan );
